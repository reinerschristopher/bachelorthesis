...
\section{Kategorientheorie} \label{seq:cat}
	...
	wurde erstmals in den vierziger Jahren von Samuel Eilenberg und Saunders Mac Lane eingeführt \cite{Eilenberg}. \\
	...
	\begin{ex}
		\textit{Passende Beispiele von Kategorien, welche später wieder genutzt werden. $\Top, \Set, (Fin)EPMet, I$} % fürr I siehe fuzzy sets, Sets einfach definiert aber viele anwendungen
	...
	\begin{defn}[Duale Kategorie]
		...
		ist durch
		\begin{equation*}
			\hom_\C(B, A) \times \hom_\C(C, B) \cong \times \hom_\C(C, B) \times \hom_\C(B, A) \mathring{\longrightarrow} \hom_\C(C, A)
		\end{equation*}
...
\section{Unscharfe Mengen} \label{seq:fuzz}
	...
	\begin{defn} %[Unscharfe Menge]
		Sei $A$ eine Menge und $\mu: A \to [0,1]$. Wir nennen des Paar $(A, \mu)$ \textit{unscharfe Menge}. 
