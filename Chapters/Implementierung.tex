...
% \section{Einleitung}
   ...
   Zunächst werden wir die Notation aus \ref{UMAP} anpassen, da wir nur das 1-Skelett der 
   topologischen Repräsentationen betrachten. Danach werden wir die Zielfunktion explizit 
   angeben und den Gradienten betrachten. 
   In Abschnitt \ref{seq:pseudo} werden wir den UMAP Algorithmus im Pseudo-Code angeben.
   Die darauf folgenden Abschnitte beschreiben rechenintensive Schritte der von uns 
   verwendeten Implementierung \cite{cpu} und beschreiben Alternativen.
   Abschnitt \ref{seq:hyper} fasst die Hyperparameter zusammen und gibt eine kurze Beschreibung an. 
...
\section{Numerische Formulierung der Optimierung} \label{seq:numerische_formulierung}
   ...
   \begin{alignat}{2}
      C(V, W) &= \sumtup{i}{j} \vij \log\left(\frac{\vij}{\wij}\right) + (1-\vij) \log\left(\frac{1-\vij}{1-\wij}\right) \\
              &= \begin{aligned}[t]
                  ...
                  \end{aligned} \\
              &=  C_v - \sumtup{i}{j} (\vij \log(\wij)) - \sumtup{i}{j} ((1-\vij) \log(1-\wij)) \label{eq:loss}
   \end{alignat}
   ...
   bzw. der Gewichte des niedrigdimensionalen Graphen, lässt sich nun der Gradient von Gleichung (\ref{eq:loss}) herleiten.
...
\section{Pseudo-Code} \label{seq:pseudo}
   ...
   betrachten ist eine effiziente Implementierung möglich, es verschiedene Verfahren gibt im sukzessive die Eigenwerte und ihre zugehörigen 
   Eigenvektoren zu finden. % TODO: Referenz für sukkzesive suche % TODO: Verschieben in section unten.
...
\section{Gradientenverfahren} \label{seq:SGD}
   Um die Funktion (\ref{eq:loss}) zu optimieren bietet sich die Wahl eines Gradientenverfahrens an, da eine differenzierbare 
   ...
   Die von uns verwendeten Implementierungen nutzen zur Minimierung von Gleichung (\ref{eq:umapgrad})

   Um die Zielfunktion zu optimieren wird im UMAP Verfahren eine Version des stochastischen Gradientenverfahren (kurz: \textit{SGD}) 
   genutzt. 
