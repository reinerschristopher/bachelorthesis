...
% \section{Einleitung}
   ...
   Wir werden dann mittels Profiling die rechenintensiven Subroutinen der von uns verwendeten 
   Implementierung \cite{cpu} identifizieren. In den darauffolgenden Abschnitten werden 
   wir die aufwendigen Routinen genauer betrachten, in dem wir in Abschnitt \ref{seq:SGD} 
...
\section{Pseudo-Code} \label{seq:pseudo}
   ...
   \Function{UMAP}{$X, N, D, d, min\_dist, n\_epochs$}  %description
      ...
      \State $Y \gets \textsc{OptimiereEinbettung}(Y, min\_dist, n\_epochs)$ \Comment{siehe Algorithmus \ref{algorithm:optimize}}
