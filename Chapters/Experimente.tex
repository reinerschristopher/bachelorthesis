...
%-----------------------------------
	...
	Wir werden reale Datensätze analysieren und das Vrehalten der Hyperparameter beschreiben, um 
	ein zusätzliches Verständnis für die von t-SNE genutzten Hyperparemeter zu bekommen 
	...
	Zusätzlich haben wir die openTSNE \cite{opentSNE} Implementierung genutzt. Diese bescheleunigt 
...
%-----------------------------------
	...
	beispielsweise t-SNE, das nicht nur paarweise die Ähnlichkeit zweier Objekte $i, j$ betrachet wird, 
	sondern stets Triple $i, j, k$. Die so erhaltene globale Struktur der Daten soll die 
	Cluster-Abstände der Daten repräsentieren. 

	
	
	Wir haben diesem Algorithmus gewühlt, da die Triplets Ähnlichkeiten mit 2-Simplizes des UMAP Verfahren haben.
	Die Triplets sind vergleichbar mit den 2-Simplizes des UMAP Verfahrens. 
	Insbesondere können die Ansätze eine lineare Teilmenge ($O(N)$) an Triplets zu finden 
