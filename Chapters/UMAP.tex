...
\section{Topologische Repräsentation} \label{sec:repr}
	...
	Die Intuition dieser Repräsentation kann wie folgt beschrieben werden. Lokale Eigenschaften der Daten werden dadurch erhalten, dass 
	wir mit Lemma \ref{lem:geod} die Geodäten auf der $X$ zugrundeliegenden Mannigfaltigkeit bestimmt haben. Für die erweiterten pseudo-metrischen 
	Räume wird dann eine geeignete unscharfe simpliziale Menge konstruiert. In Abschnitt \ref{seq:sSet} haben wir argumentiert, dass 
	es intuitiv oft genügt anstelle einer simplizialen Menge einen Simplizialkomplex zu betrachten. Deshalb können wir uns den Funktor $\fSing$ als 
	\textit{Abbildung} des erweiterten pseudo-metrischen Raumes $X_i$ auf einen unscharfen Simplizialkomplex $\mathcal{K}_i$ vorstellen, wobei jeder Simplex einen 
	Zugehörigkeitsgrad hat. Betrachten wir beispielsweise das $1$-Skelett von $\mathcal{K}_i$ so sind die $0$-Simplizes die $\mathbf{x}_j \in X$. Die 
	$1$-Simplizes beschreiben Abstände zwischen den $\mathbf{x}_j$, wobei der Zugehörigkeitsgrad eines $1$-Simplizes aus $\mathcal{K}_i$, mit Facetten 
	$\mathbf{x}_j, \mathbf{x}_l$, gerade $d_i(\mathbf{x}_j, \mathbf{x}_l)$ entspricht. 
